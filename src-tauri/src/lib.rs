...
fn extension_for(path: &Path, format: &str) -> String {
    match format {
        ...
        _ => path.extension().and_then(|value| value.to_str()).unwrap_or("png").to_ascii_lowercase(),
    }
}
...
fn safe_file_name(value: &str) -> String {
    value
        .chars()
        .map(|character| if "<>:\"/\\|?*\0".contains(character) || character.is_control() { '-' } else { character })
        .collect::<String>()
}
...
fn available_path(directory: &Path, requested_name: &str) -> Result<PathBuf, String> {
    ...
    let base = requested.file_stem().and_then(|value| value.to_str()).filter(|value| !value.is_empty()).unwrap_or("piclite");
    for index in 1..10_000 {
        let name = if index == 1 || extension.is_empty() {
            if index == 1 { safe.clone() } else { format!("{base}-{index}") }
        } else {
...
fn quantize_rgba(image: &mut image::RgbaImage, quality: u8) {
    ...
    let levels = (2.0 + 254.0 * normalized.powf(1.7)).round().clamp(2.0, 256.0);
...
fn encode_gif(original: &[u8], width: u32, height: u32, quality: u8) -> Result<Vec<u8>, String> {
    let decoder = GifDecoder::new(BufReader::new(Cursor::new(original))).map_err(|error| error.to_string())?;
    let frames = decoder.into_frames().collect_frames().map_err(|error| error.to_string())?;
    let mut encoded = Vec::new();
    {
        ...
        for frame in frames {
            ...
            encoder.encode_frame(Frame::from_parts(buffer, 0, 0, delay)).map_err(|error| error.to_string())?;
        }
    }
    Ok(encoded)
}

fn encode_static(mut image: DynamicImage, output_extension: &str, quality: u8) -> Result<Vec<u8>, String> {
    let mut encoded = Vec::new();
...
fn optimize_bytes(path: &Path, settings: &WatcherSettings) -> Result<Vec<u8>, String> {
    ...
    if source_extension == "gif" && settings.format == "keep" {
        ...
        if settings.prevent_larger && !visual_transform && candidate.len() >= original.len() {
            return Ok(original);
        }
        return Ok(candidate);
    }
    ...
    let candidate = encode_static(resized, &output_extension, settings.quality)?;
    let visual_transform = target_width != width || target_height != height || settings.format != "keep";
    if settings.prevent_larger && !visual_transform && candidate.len() >= original.len() {
        Ok(original)
    } else {
        Ok(candidate)
...
fn process_watched_file(app: AppHandle, path: PathBuf, settings: WatcherSettings, processing: Arc<Mutex<HashSet<PathBuf>>>) {
    let canonical = fs::canonicalize(&path).unwrap_or(path.clone());
    {
        let mut active = processing.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
    ...
    let result = (|| -> Result<(PathBuf, u64, u64), String> {
        ...
        let base = canonical.file_stem().and_then(|value| value.to_str()).unwrap_or("image");
        let output_path = available_path(&output_directory, &format!("{base}-piclite.{extension}"))?;
    ...
    match result {
        Ok((output_path, original_bytes, output_bytes)) => {
            ...
            event.file = canonical.file_name().and_then(|value| value.to_str()).map(str::to_string);
    ...
    processing.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).remove(&canonical);
}

#[tauri::command]
async fn select_folder(app: AppHandle, state: State<'_, DesktopState>, kind: String) -> Result<Option<String>, String> {
    let Some(selected) = app.dialog().file().set_can_create_directories(true).blocking_pick_folder() else {
        return Ok(None);
    };
    let path = selected.into_path().map_err(|error| error.to_string())?;
    let path = fs::canonicalize(&path).unwrap_or(path);
    let mut folders = state.folders.lock().map_err(|_| "文件夹状态不可用".to_string())?;
    match kind.as_str() {
...
#[tauri::command]
async fn select_images(app: AppHandle, state: State<'_, DesktopState>) -> Result<Vec<NativeImage>, String> {
    let Some(files) = app
        .dialog()
        .file()
    ...
    for selected in files {
        ...
        state.source_files.lock().map_err(|_| "文件授权状态不可用".to_string())?.insert(canonical.clone());
        images.push(NativeImage {
            name: canonical.file_name().and_then(|value| value.to_str()).unwrap_or("image").to_string(),
...
async fn read_clipboard_image() -> Result<Option<ClipboardImage>, String> {
    ...
    PngEncoder::new_with_quality(&mut png, CompressionType::Best, PngFilterType::Adaptive)
        ...
        .map_err(|error| error.to_string())?;
    Ok(Some(ClipboardImage { data: BASE64.encode(png) }))
}

#[tauri::command]
async fn export_images(payload: ExportPayload, state: State<'_, DesktopState>) -> Result<CommandResult, String> {
    let result = (|| -> Result<Vec<String>, String> {
        if payload.items.is_empty() {
            return Err("没有可导出的图片".to_string());
        }
        if !matches!(payload.mode.as_str(), "overwrite" | "same-folder" | "fixed-folder") {
            return Err("不支持的导出方式".to_string());
        }
        let fixed_folder = if payload.mode == "fixed-folder" {
            payload.fixed_folder.as_deref().map(PathBuf::from).or_else(|| {
                state.folders.lock().ok()?.export.clone()
            }).ok_or_else(|| "请先选择固定输出文件夹".to_string())?
        } else {
            PathBuf::new()
        };
        let authorized = state.source_files.lock().map_err(|_| "文件授权状态不可用".to_string())?;
        let mut paths = Vec::new();
        for item in payload.items {
            let source = item.source_path.as_deref().map(PathBuf::from);
            if matches!(payload.mode.as_str(), "overwrite" | "same-folder") {
                let Some(path) = source.as_ref() else { return Err("这张图片没有源文件路径".to_string()); };
            ...
            let target = match payload.mode.as_str() {
                ...
                "same-folder" => {
                    ...
                    available_path(source.parent().ok_or_else(|| "无法定位源文件夹".to_string())?, &item.output_name)?
                }
    ...
    Ok(match result {
        Ok(paths) => CommandResult { ok: true, paths: Some(paths), error: None },
        Err(error) => CommandResult { ok: false, paths: None, error: Some(error) },
    })
}

#[tauri::command]
async fn start_watcher(app: AppHandle, settings: WatcherSettings, state: State<'_, DesktopState>) -> Result<CommandResult, String> {
    let result = (|| -> Result<(), String> {
        if settings.input_folder.is_empty() {
            return Err("请选择来源文件夹".to_string());
        }
        let input = fs::canonicalize(&settings.input_folder).map_err(|_| "来源文件夹不存在".to_string())?;
        let folders = state.folders.lock().map_err(|_| "文件夹状态不可用".to_string())?;
        if folders.input.as_ref() != Some(&input) {
            return Err("请通过文件夹选择器确认来源位置".to_string());
        }
        let output = if settings.output_folder.is_empty() { input.join("PicLite") } else { PathBuf::from(&settings.output_folder) };
        drop(folders);

        if let Some(mut previous) = state.watcher.lock().map_err(|_| "监测状态不可用".to_string())?.take() {
        ...
        let mut watcher = notify::recommended_watcher(move |result: notify::Result<notify::Event>| match result {
            Ok(event) if matches!(event.kind, EventKind::Create(_) | EventKind::Modify(_)) => {
                for path in event.paths {
                    if !is_image(&path) || path.starts_with(&output_for_callback) || !path.starts_with(&input_for_callback) {
                        continue;
                    }
            ...
            Err(error) => emit_event(&app_handle, watcher_event("error", Some(error.to_string()))),
            _ => {}
        }).map_err(|error| error.to_string())?;
        watcher.watch(&input, RecursiveMode::Recursive).map_err(|error| error.to_string())?;
        *state.watcher.lock().map_err(|_| "监测状态不可用".to_string())? = Some(watcher);
        *state.watcher_settings.lock().map_err(|_| "监测设置不可用".to_string())? = Some(settings.clone());
        emit_event(&app, watcher_event("started", Some(format!("正在监测 {}", input.to_string_lossy()))));
        Ok(())
    })();
    Ok(match result {
        Ok(()) => CommandResult { ok: true, paths: None, error: None },
        Err(error) => CommandResult { ok: false, paths: None, error: Some(error) },
    })
}

#[tauri::command]
async fn stop_watcher(app: AppHandle, state: State<'_, DesktopState>) -> Result<CommandResult, String> {
    state.watcher.lock().map_err(|_| "监测状态不可用".to_string())?.take();
    state.watcher_settings.lock().map_err(|_| "监测设置不可用".to_string())?.take();
    emit_event(&app, watcher_event("stopped", Some("文件夹监测已停止".to_string())));
    Ok(CommandResult { ok: true, paths: None, error: None })
}

#[tauri::command]
async fn get_watcher_state(state: State<'_, DesktopState>) -> Result<WatcherState, String> {
    let active = state.watcher.lock().map_err(|_| "监测状态不可用".to_string())?.is_some();
    let settings = state.watcher_settings.lock().map_err(|_| "监测设置不可用".to_string())?.clone();
