...
- 右下角悬浮结果：桌宠 / 紧凑 / 完整三种入口；桌宠可选绿色或黑色猫帽，支持拖动、滚轮缩放、点击互动、双击选图、右键菜单和透明无边框常驻
...
标准系统托盘 API 在 Windows、macOS 和 Linux 上没有统一的文件拖放事件，因此 PicLite 使用托盘菜单唤起一个无任务栏图标、始终置顶的“悬浮压缩坞”。桌宠模式提供绿色苦力怕猫帽和黑色末影人猫帽两只角色：左键拖动位置、滚轮调整 60%–180% 大小、单击触发跳跃 / 压扁回弹 / 左右抖动与随机对白，双击选择图片，右键可切换角色与大小、开关置顶、打开主窗口或退出。压缩完成或文件夹监测产生结果后，会在当前屏幕右下角展开结果卡片；画质与尺寸每次都从原图重算，支持回撤、预览、复制压缩文件、定位文件，并可设置 0–120 秒后自动返回桌宠。
