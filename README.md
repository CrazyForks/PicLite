...
- 拖放、多文件选择和剪贴板粘贴导入
- 结果图一键复制到系统剪贴板，可直接粘贴到聊天、文档和设计软件，无需先导出文件
...
- 托盘唤起的磨砂置顶悬浮压缩坞：分别调节画质和尺寸、显示真实节省比例，并支持回撤与复制结果图
- 本机结果图库：保留压缩预览、体积、像素、本地文件位置和图床链接，可再次复制、定位或上传
- 桌面端可选上传到 WebDAV、Cloudflare R2、阿里云 OSS、FTP 或 SFTP，上传后自动复制公开图片链接
- 自动恢复上次画质、缩放、水印、监测配置，并支持保存和删除自定义压缩预设
- 桌面专属应用设置：默认输出规则、固定目录、后缀、覆盖确认、托盘行为和关于页面
- Docker 自托管和 GitHub Actions 跨平台自动构建

图片默认只在浏览器或桌面客户端本地处理，不会上传到 PicLite 服务器。只有你主动点击“上传图床”时，结果图才会发送到你自己配置的存储服务；密码与 Secret Key 只保留在当前运行内存中，关闭 PicLite 后立即丢弃。网页端受浏览器权限限制，持续文件夹监测和图床上传只在桌面客户端提供。
...
标准系统托盘 API 在 Windows、macOS 和 Linux 上没有统一的文件拖放事件，因此 PicLite 使用托盘菜单唤起一个无任务栏图标、始终置顶的磨砂“悬浮压缩坞”。把文件拖到压缩坞即可直接压缩；画质与尺寸可分别连续调整，每次应用都从原图重新计算，避免多次有损叠压。压缩坞支持回撤上一组参数、复制结果图，并始终生成新文件，不会覆盖原图。

### 图床上传与图库

在桌面端“应用设置 → 图床上传”中选择 WebDAV、Cloudflare R2、阿里云 OSS、FTP 或 SFTP，填写服务地址、远端目录和公开访问地址。工作台右上角的上传按钮和图库中的“上传图床”都会上传当前压缩结果，并在成功后复制图片链接。
...
非敏感配置会保存在本机偏好中；密码、私钥口令和 Secret Key 不会持久化。图库数据保存在本机 WebView 的 IndexedDB 中，“移除”只删除图库记录，不会删除真实文件或远端对象。
