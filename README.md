...
- 拖放、多文件选择和剪贴板粘贴导入；桌面端可选监听剪贴板，新复制图片会按当前参数自动压缩并把结果放回剪贴板
- 桌面端复制的是实际压缩文件而不是解码后的位图，粘贴为附件时保持 JPEG / WebP / PNG 的真实小体积
...
- 文字水印：原生读取并显式载入系统字体或导入字体文件，支持角度、字号、透明度、0–100% 非线性铺设密度、自由位置和阴影
...
- 右下角悬浮结果：桌宠 / 紧凑 / 完整三种入口；白色小鸡外套猫与黑色末影猫自动跟随主题，支持原地缩放、拖动、眨眼、呼吸、肚子文字、点击互动和右键菜单
- 本机结果图库：保留压缩预览、体积、像素、本地文件位置和图床链接，可再次复制、定位或上传
...
桌面版基于 Tauri，不再打包完整 Chromium；Windows 使用系统 WebView2，macOS 使用 WebKit，Linux 使用 WebKitGTK。安装包会因平台、SFTP 加密库与打包格式而变化，实际大小请以 Releases 页面为准。
...
标准系统托盘 API 在 Windows、macOS 和 Linux 上没有统一的文件拖放事件，因此 PicLite 使用托盘菜单唤起一个无任务栏图标、始终置顶的“悬浮压缩坞”。桌宠模式会按悬浮窗主题自动显示白色小鸡外套猫或黑色末影猫：左键拖动位置、滚轮在当前位置调整 60%–180% 大小，单击触发跳跃 / 压扁回弹 / 左右抖动、眨眼、呼吸、肚子文字与随机对白；右键可调整大小、选择图片、开关置顶、打开主窗口或退出。压缩完成、剪贴板监听或文件夹监测产生结果后，会在当前屏幕右下角展开结果卡片；画质、尺寸和 JPG / PNG / WebP / 原格式选项每次都从原图重算，支持回撤、预览、复制压缩文件、定位文件，并可设置 0–120 秒后自动返回桌宠。

### 图床上传与图库

在桌面端“应用设置 → 图床上传”中选择通用 S3 / MinIO、WebDAV、Cloudflare R2、阿里云 OSS、FTP 或 SFTP，填写服务地址、远端目录和公开访问地址，然后点击“保存到本机”。工作台右上角的上传按钮和图库中的“上传图床”都会上传当前压缩结果，并在成功后复制图片链接。

- WebDAV 使用 HTTPS + Basic Auth，建议服务端强制 HTTPS。
- 通用 S3 使用 Signature V4；AWS S3 可关闭 Path-style，MinIO 和多数自建兼容服务通常开启 Path-style。
- R2 使用 S3 Signature V4；Region 通常填写 `auto`。
- OSS 使用 AccessKey 签名；建议为 PicLite 创建只有指定 Bucket 写入权限的子账号。
- FTP 是明文协议，仅建议在受信任内网使用；公网请使用 SFTP。
- SFTP 会校验系统 `~/.ssh/known_hosts`，首次使用前先在终端执行一次 `ssh 用户名@服务器` 并核对服务器指纹。

上传配置与凭证会在你点击保存后写入当前系统用户的 PicLite 配置目录；Unix 系统使用仅当前用户可读写的文件权限。图库数据保存在本机 WebView 的 IndexedDB 中，“移除”只删除图库记录，不会删除真实文件或远端对象。
